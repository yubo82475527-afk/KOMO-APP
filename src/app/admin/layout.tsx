...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (viewer.state === "signed_out") {
    return (
      <AdminShell viewer={{ fullName: "未登录", roles: [] }}>
        <div className="mx-auto max-w-md rounded-lg border border-[#d7dee7] bg-white p-5 shadow-sm">
          ...
          <div className="mt-4">
            <AuthCard redirectTo="/admin/overview" />
          </div>
        </div>
      </AdminShell>
    );
  ...
  if (viewer.state === "error") {
    return (
      <AdminShell viewer={{ fullName: "加载失败", roles: [] }}>
  ...
  if (!canUseAdmin) {
    return (
      <AdminShell viewer={{ fullName: viewer.profile.fullName, roles: viewer.roles }}>
        <div className="rounded-lg border border-[#ffd6d6] bg-white p-5 shadow-sm">
  ...
  const scope = await getAdminShellScope();

  return (
    <AdminShell locale={viewer.locale} viewer={{ fullName: viewer.profile.fullName, roles: viewer.roles }} scope={scope}>
      {children}
    </AdminShell>
  );
}
