...
const leaveTypeOptions = ["年假", "事假", "病假", "调休", "其他"];
...
export function LeaveApplyView({ data }: { data: Extract<LeaveApplyPageData, { state: "ready" }> }) {
  ...
  const [message, setMessage] = useState(`当前生效模板：${data.activeTemplateName ?? "暂未配置"}`);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      setStatus("error");
      setMessage(payload.error ?? "请假申请提交失败，请稍后重试。");
      return;
    }

    setStatus("success");
    setMessage("请假申请已提交，正在跳转到审批详情...");
  ...
  return (
    <MobileShell active="approval">
      <section className="rounded-3xl bg-white p-5 shadow-sm">
        <p className="text-sm text-[#607089]">发起请假申请</p>
        ...
        <p className="mt-2 text-sm leading-6 text-[#607089]">
          {data.viewer.departmentName ?? "未分配部门"} · 审批会按当前模板自动流转到对应审批人。
        </p>
      </section>
      ...
      <Section title="填写申请">
        <form className="space-y-4" onSubmit={handleSubmit}>
          <label className="block">
            <span className="text-sm text-[#607089]">请假类型</span>
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              <span className="text-sm text-[#607089]">开始日期</span>
            ...
            <label className="block">
              <span className="text-sm text-[#607089]">结束日期</span>
          ...
          <label className="block">
            <span className="text-sm text-[#607089]">请假天数</span>
          ...
          <label className="block">
            <span className="text-sm text-[#607089]">请假原因</span>
            <textarea value={reason} onChange={(event) => setReason(event.target.value)} rows={5} className="mt-2 w-full rounded-xl border border-[#d9dee8] px-3 py-3 text-sm" placeholder="请填写请假原因" required />
          </label>
          ...
          <button type="submit" disabled={status === "submitting"} className="w-full rounded-2xl bg-[#184e77] py-4 font-semibold text-white shadow-sm disabled:bg-[#8a97a8]">
            {status === "submitting" ? "提交中..." : "提交请假申请"}
          </button>
        </form>
      </Section>
    </MobileShell>
