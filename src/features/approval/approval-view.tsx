            ...
            (tab === "mine" ? data.myRequests : data.pendingApprovals).map((item) => (
              <Link key={item.id} href={`/approval/${item.id}`} className="block rounded-2xl bg-[#f6f8fb] p-4">
                ...
                <div className="mt-3 grid grid-cols-2 gap-3 text-sm text-[#526174]">
                  <p>{dictionary.approval.leaveType}：{item.payload.leaveType}</p>
                  <p>{dictionary.approval.days}：{item.payload.days}</p>
                  <p>{dictionary.approval.startDate}：{item.payload.startDate}</p>
                  <p>{dictionary.approval.endDate}：{item.payload.endDate}</p>
                </div>
                <p className="mt-3 text-sm text-[#607089]">{dictionary.approval.currentStep}：{item.currentStepName ?? dictionary.common.success}</p>
                {item.currentApproverName ? <p className="mt-1 text-sm text-[#607089]">{dictionary.approval.currentApprover}：{item.currentApproverName}</p> : null}
                <p className="mt-2 text-xs text-[#8a97a8]">{dictionary.approval.submittedAt}：{item.submittedAt ? formatDateTime(item.submittedAt, data.locale, { year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hour12: false }) : dictionary.common.notSet}</p>
              </Link>
            ))
          ) : (
